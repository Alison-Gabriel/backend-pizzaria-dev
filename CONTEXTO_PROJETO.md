...
│   │   ├── validateAdminRole.ts        # Verifica se usuário é admin
│   │   ├── validateAccessToken.ts # Valida JWT token
...
- **Middlewares**: `<description>.ts` (ex: `validateAccessToken.ts`)
...
### 1. **validateAccessToken** (`middlewares/validateAccessToken.ts`)
...
### 2. **validateAdminRole** (`middlewares/validateAdminRole.ts`)

**Função**: Verifica se o usuário autenticado tem permissão de ADMIN.

**Pré-requisito**: Deve ser usado **após** o middleware `validateAccessToken`.

**Fluxo**:

1. Obtém `user_id` do `req` (adicionado pelo `validateAccessToken`)
...
**Middlewares**: `validateAccessToken`, `validateAdminRole`, `validateSchema(createCategorySchema)`
...
1. POST /category
   ↓
2. Middleware: validateAccessToken
   - Valida token JWT
   - Adiciona user_id ao req
   - Se inválido → 401
   ↓
3. Middleware: validateAdminRole
...
- Rotas protegidas por middlewares `validateAccessToken` e `validateAdminRole`
